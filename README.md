...
- 排程：台灣時間每天09:30、16:00、22:00；Cloudflare Worker 每5分鐘檢查該時段
  正式 LINE 投遞收據，於時段後5分鐘至5小時50分鐘內必要時自動補觸發
- 網站：GitHub Pages
...
- Threads：合併官方 `keyword_search`、`THREADS_ACCESS_TOKEN`、
  ...
  3房以上、最近2天來源活動及非包租代管／仲介同業，租金、坪數與照片可未提供
- Facebook：程式不登入、不使用帳密／Cookie／Session；資料來源為
  ...
  最近2天來源活動及非包租代管／仲介同業。FB 另採語境判斷：公司、證照、
  服務費或招攬委託等業者廣告排除，但「屋主想找代管／人在外地／沒時間管理」
  會保留並列為高價值訊號。30至35坪以上、租金、照片與屋主訊號只用於評分
- 來源時間窗口：591保留本輪完整驗證仍在刊登的全部符合條件物件，不限來源新增／更新日期；
  FB、樂屋網、Threads、信義房屋與永慶房屋只保留來源新增或更新在最近2天內的物件。
  所有來源仍須本輪重新驗證成功才會發布。
- 新房源：與前一輪已發布的 `latest.json` 比較（包括未發LINE的維護版）。591以來源ID比對，前輪未出現的物件標記24小時，期間跨版保留；價格改變不改房源身分，降價另標示。其他來源仍依首次發現24小時判定。
- 顯示：每個來源均為單欄列表、每筆左圖右文；FB提供「全部、A級、B級、C級」
  屋主房源雷達，Threads維持「全部、高符合、30坪以上、資訊待補」與自動符合度，
  其他來源維持各自的分類與排序控制。
  新房源標記出現在首圖上，不以日曆換日重設24小時計時。
...
- 591 若明確遭遇 403／429（包括已有部分成功資料），正式流程預設等待10分鐘，
  最多4次從未完成分頁續抓；同輪90分鐘內的 checkpoint 會依物件ID去重合併，
  四區完整前不部署。`crawl_policy=active-all-v1` 防止舊兩天範圍的 checkpoint 被當作全量完成。變數
  `RENTAL_591_RETRY_DELAY_SECONDS` 可在300至1200秒間調整，
