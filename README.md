...
- Threads：合併官方 `keyword_search`、`THREADS_ACCESS_TOKEN`、
  ...
  3房以上及非包租代管／仲介同業；本輪API或人工入口仍可取得的現有貼文不受刊登日期限制，租金、坪數與照片可未提供
- Facebook：程式不登入、不使用帳密／Cookie／Session；資料來源為
  ...
  非包租代管／仲介同業。本輪資料入口仍可取得的現有貼文不受刊登日期限制。FB 另採語境判斷：公司、證照、
  服務費或招攬委託等業者廣告排除，但「屋主想找代管／人在外地／沒時間管理」
  會保留並列為高價值訊號。30至35坪以上、租金、照片與屋主訊號只用於評分
- 現有物件：所有來源都以本輪來源列表、官方API或已授權入口仍可取得且通過條件的物件為準，
  不再因刊登／更新時間較舊或未提供時間而刪除；所有來源仍須本輪重新驗證成功才會發布。
