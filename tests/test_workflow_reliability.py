...
class WorkflowReliabilityTests(unittest.TestCase):
    ...
    def test_rate_limit_uses_delayed_fresh_runner_retry(self) -> None:
        ...
        self.assertIn("retry_591_validation_8:", self.workflow)
        ...
        self.assertIn("fresh-rental-validation-retry-8", self.workflow)
        ...
        self.assertIn("--attempt retry8=.validation/retry8", self.workflow)
        self.assertIn("--require-complete-591", self.workflow)
        self.assertIn("--require-complete-yungching", self.workflow)
        self.assertIn("RENTAL_591_RETRY_DELAY_SECONDS || '300'", self.workflow)
