...
1. FB與Threads不使用帳號密碼、Cookie或瀏覽器Session；指定四區與3房以上為
   硬條件。FB排除包租代管同行、保留一般房仲為C級；591刊出本輪現刊、其他來源近7天內資料。
...
7. 591 不限刊登日期，其他來源七天的來源時間驗證與跨來源去重；真正降價物件可重新顯示。
...
def filter_source_freshness(
    ...
    """Keep every live-validated 591 listing; require seven-day activity elsewhere."""
    ...
    for source, source_stats in source_rows.items():
        ...
        source_stats["freshness_policy"] = "active_listing" if freshness_days is None else "source_activity"
        source_stats["active_validated" if freshness_days is None else f"fresh_within_{freshness_days}_days"] = 0
    ...
    for item in items:
        ...
        fresh_key = "active_validated" if freshness_days is None else f"fresh_within_{freshness_days}_days"
        timestamp = source_listing_time(item)
        if freshness_window is None:
            # Liveness comes from collect_591_listings, never from an old timestamp
            # or a snapshot. Do not manufacture an update date when none is shown.
            item.source_timestamp = timestamp.isoformat() if timestamp else ""
...
<header>
  <div class="wrap">
    ...
    <p class="subtitle">六個來源分區顯示；591保留本輪重新驗證仍在刊登的全部符合條件物件，不限新增／更新日期；其他來源保留7天內物件。</p>
...
<main class="wrap">
<div class="notice">每個來源每輪都會重新檢查，不沿用舊快照。591不限新增／更新日期，仍須符合四區、整層住家、4房以上等原有條件；其他來源須有最近7天內的來源時間。「新房源」比對前一輪已發布頁面，591新出現的房源標記保留24小時；租金變動另以降價標示。</div>
