...
Worker 每5分鐘檢查一次，並在各時段後5分鐘至5小時50分鐘的備援窗口內確認：
...
渲染端點每次只接受一個白名單內的列表或詳細頁，解析工作留在GitHub Runner，避免
Worker整輪解析超出CPU限制。列表保留JavaScript與必要等待以通過公開頁面的前端中介；
詳細頁只讀伺服器已輸出的資訊，停用JavaScript，等待主標題出現，並不下載樣式、圖片、
影音與字型等資源。回傳HTML仍保留該物件自己的相簿網址，因此照片驗證規則不變，同時
大幅降低每輪Browser Run時間，讓每日三個發送時段可共用帳戶級額度。
429 依 `Retry-After`／Browser Run limits 等待，至少21秒、單次最多60秒；
每日額度耗盡（包括Worker的 `browser_daily_quota` 錯誤碼）不會快速重試。後續失敗保留本輪已驗證項目並回 `partial`；
